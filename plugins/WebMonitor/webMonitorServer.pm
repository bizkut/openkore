...
# Show cart
	...
	if ($char->cartActive) {
		...
		for (my $i = 0; $i < $char->cart->size; $i++) {
			...
			$itemName .= T(" -- Not Identified") if !$item->{identified};
...
# Show storage
	...
	for (my $i = 0; $i < $char->storage->size; $i++) {
		...
		if ($item->usable) {
			...
			push @storageUsableGetButton, '<td><a class="btn btn-mini btn-inverse" href="/handler?csrf=' . $csrf . '&command=storage+get+' . $i . '">' . T('Get') . '</a></td><td></td>' if ($char->storage->isReady);
			...
			push @storageEquipmentGetButton, '<td><a class="btn btn-mini btn-inverse" href="/handler?csrf=' . $csrf . '&command=storage+get+' . $i . '">' . T('Get') . '</a></td><td></td>' if ($char->storage->isReady);
		} else {
